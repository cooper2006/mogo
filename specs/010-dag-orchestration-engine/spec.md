...
- FR-1: 支持四编排模式：sequential / supervisor / hybrid / graph；**节点间数据传递 = 上游节点输出写入共享上下文（key 引用），下游按 key 读取；supervisor 模式由监督节点分派子节点并聚合其结果（分派/聚合边界由编排定义声明）**
- FR-2: graph 模式按 DAG 拓扑执行，无依赖节点可并行；**并行默认并发度 = 4（`dag_max_concurrency` 可配），超限节点排队等待（不拒绝）**
- FR-3: 执行前拓扑排序 + 环检测，环报错含环路径；**环路径输出格式 = 节点 ID 序列（如 `A → B → C → A`），定位到具体节点**
- FR-4: 节点支持条件跳过（表达式求值），跳过可追溯；**表达式 = JSON 条件对象（`==`/`!=`/`>`/`>=`/`<`/`<=`/`and`/`or`/`not`/`in`/`has`），禁用任意代码执行**；语法错误默认 fail_closed（跳过 + 审计），可配置为报错中断
- FR-5: 节点支持指数退避重试，次数/退避可配置；**节点重试包裹上游 007 的模型调用重试（分层，不重复退避）**
- FR-6: 节点失败阻塞其下游依赖；**"阻塞" = 全部传递下游闭包（非仅直接下游），被阻塞节点状态标记为 `blocked`（区别于 `failed`）**
- FR-7: 编排执行事件（节点启动/完成/失败/跳过/重试）进审计；**跳过事件的追溯内容含：跳过原因（求值结果）、被跳过的下游节点标记**
- FR-8: 编排定义声明式（节点/边/条件/重试），可复用、可版本化（与 002 工作流版本化对接）；**版本字段 = 编排定义含 `version`，旧版本保留可回看**
- FR-9: 现有"内容规划/研究模式"可迁移到通用 DAG 引擎（向后兼容）；**迁移策略 = 双轨并行 + 等价测试（新编排走 DAG，旧场景保留至等价验证通过）**
- FR-10: **节点为原子成功/失败（不可中断的中间态），节点内多步任一失败即节点失败**
- FR-11: **supervisor 模式的失败传播：监督节点本身失败 → 整个编排失败；子节点全失败 → 监督节点失败（按数量阈值可配）**
