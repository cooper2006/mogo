...
- [x] CHK002 五事件各自的"可携带/可修改"数据是否定义（SessionStart 可注入什么上下文，SessionEnd 可清理什么）？
...
- [x] CHK009 与 001（gatekeeper）"PreToolUse 是 001 六层门禁链的扩展点"（Notes/FR-5）——001 是否反向声明了 009 钩子作为其第 4 层（审批）的拦截扩展（两边扩展点关系是否双向一致）？
- [x] CHK010 与 002（session-versioning）"会话生命周期事件是 002 的钩子载体"（FR-7）——002 是否声明 SessionStart/End/MemoryCommit 经 009 触发（同 002 CHK010）？
- [x] CHK011 与 019（harness-elastic）"薄模式可跳过非红线 Hooks"（019 clarify OQ-2）——009 是否声明"哪些钩子是薄模式可跳过的、fail_closed 底线是否保留"（两边口径）？
- [x] CHK012 "钩子进审计通道（与 001 联动）"（FR-5）——审计落点是 001 的审计层还是独立钩子审计集合（与 001 OQ 是否对齐）？
...
- [x] CHK016 钩子执行对工具调用延迟的影响（5s 超时 × 多钩子叠加）是否有总延迟预算（spec 是否声明端到端延迟上限）？
