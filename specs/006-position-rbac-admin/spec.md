...
**Input**: 回溯固化 MOGO 既有优势能力（规划文档 §1.1"管理后台：组织/用户/角色/模型/知识/Skill/工具/审计/运行状态、RBAC"）：岗位角色 CRUD 与启停、用户-角色绑定（主角色 + 多角色）、角色能力开关与工具/Skill 访问模式（all/selected + 显式 ID 列表）、全能力管理员内置角色、角色操作全量审计。
...
- **与 017（three-scope-memory）**：006 的"**全能力管理员**"角色具备**组织级记忆提升授权权**（017 FR-4 的范围升级授权）——006 的角色能力维度含记忆提升权，017 的范围升级经 006 RBAC 授权。
- **与 019（harness-elastic-config）**：019 的厚度 profile 配置权**受 006 RBAC 约束**（仅授权管理员可配厚度，见 019 FR-9）。
