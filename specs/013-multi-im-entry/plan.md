...
新增 `services/chat-api/app/im_gateway/` 子模块：IM 渠道适配层（每渠道一个 adapter）、IM↔MOGO 会话映射、统一路由。核心原则（CubePlex）：IM 只是入口，能力面复用既有 Workspace/Sandbox，不新建能力。首期仅接入 1 渠道，多渠道按节奏补齐。
...
**Storage**: 新增 `im_channels`（渠道配置/bot token 指针）+ `im_session_bindings`（IM 会话 ↔ MOGO 会话映射）
...
└── im_gateway/
    ...
    │   ├── slack.py
    │   └── teams.py
    ├── session_mapping.py      # IM 会话 ↔ MOGO 会话
    └── router.py              # 多渠道统一路由
...
- OQ-3 渠道会话纳入 002 版本化：**是**——IM 会话映射到 MOGO 会话（`im_session_bindings`），002 的 commit/share 对 IM 会话同样生效。
