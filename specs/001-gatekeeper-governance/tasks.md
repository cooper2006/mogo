...
- [ ] T015 [P] 实现 `risk.py`：R0–R4 工具风险分级注册（`risk_tiers` 表）
- [ ] T016 [P] 实现 `AUTONOMY_MATRIX`（25 格，`autonomy_matrix` 表）：`[L][R] -> allow/require_approval/deny`，含"R4 任意 L = deny、管理员不可覆盖"规则
- [ ] T017 矩阵单元测试：断言 L3×R2=require_approval、L5×R4=deny、R0 全 L=allow 三组 Acceptance
- [ ] T018 实现 `api/routes.py` 矩阵/风险分级 CRUD 端点（管理面维护矩阵 + 工具风险级）
...
- [ ] T019 [P] 实现 `pii.py`：正则识别器（私钥/身份证/银行卡/手机号/邮箱 5 类）+ 策略引擎（mask/remove/hash/abstract）
- [ ] T020 [P] 实现 PII 策略表 `pii_policies`：全局默认（手机号 mask/私钥 remove/身份证 hash/银行卡 mask/邮箱 abstract）+ 租户级可覆盖（覆盖需 006 授权 + 001 审计）
- [ ] T021 实现 `layers/redaction.py`（层 3：请求/响应文本脱敏，作用文本不改工具执行语义）
- [ ] T022 PII 单测：5 类 PII × 4 策略矩阵 + 审计 0 明文断言（验收：手机号 mask / 私钥 remove）
...
- [ ] T023 [P] 扩展 `rbac_model.py`：权限码三级隔离（租户/组织/用户）+ 岗位角色预设组映射（复用 T009）
- [ ] T024 实现权限码管理端点（`api/routes.py`：创建/授权/回收权限码，变更即时生效 + 审计）
- [ ] T025 US4 集成测试：Acceptance 三组（knowledge:read 通过 / admin:role:assign 无码拒绝 / 岗位角色绑定预设组生效）
...
- [ ] T026 [P] 实现 `quota.py` + `quota_counters`（MongoDB 原子 findOneAndUpdate，三维 = 租户/用户/工具，时间窗口 min/day/month，不引入 Redis）
- [ ] T027 实现 `layers/quota.py`（层 5：超限拒绝 + 返回超限维度）
- [ ] T028 US5 集成测试：租户日配额触发 + 单工具配额独立触发两组 Acceptance
...
- [ ] T029 [P] 门禁链性能验证：单次 evaluate p95 < 15ms（不含审批等待），审计落库异步（性能目标断言）
- [ ] T030 [P] 声明式配置收尾：`config.py` 支持增删门禁层无需改代码 + 配置变更留审计（FR-10）
- [ ] T031 审计覆盖率自检：全链路工具调用 100% 经门禁 + 100% 落 `gate_events`（Success Criteria 总验收）
- [ ] T032 写 `quickstart.md`（门禁启用 + 各故事验证步骤）+ `contracts/gatekeeper.md`（六层 IO 契约 + 25 格矩阵定义）
