...
- FR-11: **降级事件"原因"枚举 = 上游 429 / 上游 5xx / 超时 / 手动切换（4 类枚举值，非自由文本），落 `token_usage_logs.degradation_reason`**
- FR-12: **重试响应取消信号：重试期间若收到会话取消/网关超时信号，立即中止重试并返回取消错误（不退避等待）**
- FR-13: **failover 切换成功时的计量归属：按实际命中供应商/模型分别记录（切换前的失败调用记 `failover_from`，成功调用记 `failover_to` 的实际用量与成本，不合并归到单一维度）**
...
- **与 010（dag-orchestration）**：007 的**模型调用级重试**被 010 的**节点级重试包裹**（分层）——010 节点失败重跑节点时，节点内模型调用走 007 的 provider failover + 退避；二者不重复退避（见 010 FR-5）。
- **与 001（gatekeeper）**：007 的配额维度是 **LLM 调用（token/成本）**，001 的配额是**工具调用次数**——二者维度不同但概念一致（都是用量配额，007 管 token 侧，001 管工具调用侧）；007 不做成本预算硬限流（限流决策由 001 治理层负责）。
- **与 008（ops-dashboard）**：007 写入 `token_usage_logs` 的韧性字段（`failover_from`/`failover_to`/`degradation_step`）供 008 质量看板归因（见 008 FR-8）。
