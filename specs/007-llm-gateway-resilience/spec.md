...
- FR-1: LLM 调用支持主/备供应商 failover，失败自动切换并记录
- FR-2: 支持 degradation_chain 逐级降档，链耗尽返回明确错误
- FR-3: 可重试错误（429/5xx/超时）指数退避 + 抖动重试，次数与退避上限可配置
- FR-4: 不可重试错误（401/403）不重试直接失败
- FR-5: 每次调用记录 token 用量（输入/输出）与估算成本
- FR-6: 用量/成本按供应商/模型/租户/智能体维度聚合，供特性 008 驾驶舱消费
- FR-7: failover/降级/重试/计量事件全部日志可观测
