...
- [ ] T001 创建 admin-api/app/services/capability_assets/ 包骨架 + 子模块
...
- [ ] T005 实现能力扫描（REST/MCP 契约抽取）
- [ ] T006 实现资产注册（契约四段 + 版本 + owner）
- [ ] T007 实现扫描去重（端点+方法）
- [ ] T008 US1 测试：发现 + 注册 + 去重

## Phase 4: User Story 2 (P2) — 治理视图 + 状态管理

- [ ] T009 实现全量资产治理视图（列表 + 详情下钻）
- [ ] T010 实现状态管理（active/deprecated/offline + 审批）
- [ ] T011 实现 a2a_exposed 标记（供 012 生成 AgentCard）
